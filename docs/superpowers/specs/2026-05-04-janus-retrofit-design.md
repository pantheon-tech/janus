...
**Status:** approved (rev 10 — addresses code-review iterations 1–9; reviewer score 8.6/10)
**Date:** 2026-05-04
...
- For `monorepo-root` retrofit specifically: because `install-deps` is omitted, the resulting branch contains **no `pnpm-lock.yaml`**. The user's first `pnpm install` post-retrofit generates it. Documented in plan stdout for the monorepo case so users aren't surprised by the missing lockfile in the PR diff.
...
5. `merge-claude-kit` — category containing the substeps below; **each substep is a discrete step (one commit) with the bare id shown** (`claude-settings-merge`, `claude-md-snapshot`, `claude-skills-overlay`, `claude-hooks-overlay`, `claude-misc-overlay`), no `merge-claude-kit/` prefix. Within-category alphabetical ordering applies among them.
...
- **`json_merge`:** not emitted by v0.1 plan-builder; reserved in the §7 op vocabulary for future use.
